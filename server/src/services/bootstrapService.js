...
const alterTables = async () => {
  const statements = [
    ...
    `ALTER TABLE payments ADD COLUMN IF NOT EXISTS reference_id INTEGER`,
    `ALTER TABLE payments ADD COLUMN IF NOT EXISTS provider_payload JSONB DEFAULT '{}'::jsonb`,
    `ALTER TABLE room_service ADD COLUMN IF NOT EXISTS booking_id INTEGER REFERENCES bookings(id) ON DELETE SET NULL`,
